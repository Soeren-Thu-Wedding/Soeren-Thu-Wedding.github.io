    ...
    with open(path, 'r') as csvfile:
        ...
        for row in reader:
            if first_row:
                first_row = False
                continue
            party_name, first_name, last_name, party_type, language, is_child, category, is_invited, email = row[:9]
            if not party_name:
            ...
            party.save()
            if email:
                guest, _ = Guest.objects.get_or_create(party=party, email=email)
...
def export_guests():
    headers = [
        'party_name', 'first_name', 'last_name', 'party_type', 'language',
